<template>
  <div class="wrapper-content">
    ...
    <div class="list" v-if="itemsData">
      <ProductsStockStatusItem class="list-item" v-for="(item, keyItem) in itemsData"
                               :key="$root.guid()"
                               :item="item"
      ...
      </ProductsStockStatusItem>
    </div>
  </div>
</template>

<script>
import ProductsStockStatusItem from "./ProductsStockStatusItem.vue";
import api from "../../common/api";

export default {
  name: "ProductsStockStatus",
  components: {
    ProductsStockStatusItem,
  },
  data() {
  ...
  methods: {
    ...
    addItem() {
      this.itemsData.unshift({'name': '', 'description': '', 'delivery_time': '',})
    },
    getItemsData() {
      this.loading = true;
